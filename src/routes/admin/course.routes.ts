...
router.route('/')
    .post(auth(USER_TYPE.ADMIN), upload.fields([{name: "video", maxCount: 100 }]), validate(validation.createCourse), controller.createCourse)
    .get(auth(USER_TYPE.ADMIN), validate(validation.getCourse), controller.getCourse)
    .put(auth(USER_TYPE.ADMIN), upload.fields([{name: "video", maxCount: 100 }]), validate(validation.updateCourse), controller.updateCourse)
